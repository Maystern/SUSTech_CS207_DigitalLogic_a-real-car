set_property IOSTANDARD LVCMOS33 [get_ports rx]
set_property IOSTANDARD LVCMOS33 [get_ports sys_clk]
set_property IOSTANDARD LVCMOS33 [get_ports tx]

set_property PACKAGE_PIN N5 [get_ports rx]
set_property PACKAGE_PIN P17 [get_ports sys_clk]

set_property PACKAGE_PIN T4 [get_ports tx]

set_property PACKAGE_PIN P5 [get_ports reverse_signal]
set_property IOSTANDARD LVCMOS33 [get_ports reverse_signal]


set_property PACKAGE_PIN P5 [get_ports reverse_signal]
set_property IOSTANDARD LVCMOS33 [get_ports reverse_signal]

set_property PACKAGE_PIN P4 [get_ports clutch_signal]
set_property IOSTANDARD LVCMOS33 [get_ports clutch_signal]

set_property PACKAGE_PIN P3 [get_ports brake_signal]
set_property IOSTANDARD LVCMOS33 [get_ports brake_signal]

set_property PACKAGE_PIN P2 [get_ports throttle_signal]
set_property IOSTANDARD LVCMOS33 [get_ports throttle_signal]

set_property PACKAGE_PIN R15 [get_ports start_engine_signal]
set_property IOSTANDARD LVCMOS33 [get_ports start_engine_signal]

set_property PACKAGE_PIN P15 [get_ports stop_engine_signal]
set_property IOSTANDARD LVCMOS33 [get_ports stop_engine_signal]

#set_property PACKAGE_PIN L1 [get_ports manual_not_starting]
#set_property IOSTANDARD LVCMOS33 [get_ports manual_not_starting]

#set_property PACKAGE_PIN M1 [get_ports manual_starting]
#set_property IOSTANDARD LVCMOS33 [get_ports manual_starting]

#set_property PACKAGE_PIN K3 [get_ports manual_moving]
#set_property IOSTANDARD LVCMOS33 [get_ports manual_moving]

set_property PACKAGE_PIN K1 [get_ports engine_power]
set_property IOSTANDARD LVCMOS33 [get_ports engine_power]

set_property PACKAGE_PIN V1 [get_ports turn_left_signal]
set_property IOSTANDARD LVCMOS33 [get_ports turn_left_signal]

set_property PACKAGE_PIN R11 [get_ports turn_right_signal]
set_property IOSTANDARD LVCMOS33 [get_ports turn_right_signal]

set_property PACKAGE_PIN U4 [get_ports go_straight_signal]
set_property IOSTANDARD LVCMOS33 [get_ports go_straight_signal]


set_property PACKAGE_PIN J4 [get_ports turn_right]
set_property IOSTANDARD LVCMOS33 [get_ports turn_right]

set_property PACKAGE_PIN G4 [get_ports turn_left]
set_property IOSTANDARD LVCMOS33 [get_ports turn_left]

set_property PACKAGE_PIN J3 [get_ports manual_state]
set_property IOSTANDARD LVCMOS33 [get_ports manual_state]

set_property PACKAGE_PIN J2 [get_ports semiauto_state]
set_property IOSTANDARD LVCMOS33 [get_ports semiauto_state]

set_property PACKAGE_PIN K2 [get_ports auto_state]
set_property IOSTANDARD LVCMOS33 [get_ports auto_state]

set_property PACKAGE_PIN G2 [get_ports seg_en[7]]
set_property IOSTANDARD LVCMOS33 [get_ports seg_en[7]]
set_property PACKAGE_PIN C2 [get_ports seg_en[6]]
set_property IOSTANDARD LVCMOS33 [get_ports seg_en[6]]
set_property PACKAGE_PIN C1 [get_ports seg_en[5]]
set_property IOSTANDARD LVCMOS33 [get_ports seg_en[5]]
set_property PACKAGE_PIN H1 [get_ports seg_en[4]]
set_property IOSTANDARD LVCMOS33 [get_ports seg_en[4]]
set_property PACKAGE_PIN G1 [get_ports seg_en[3]]
set_property IOSTANDARD LVCMOS33 [get_ports seg_en[3]]
set_property PACKAGE_PIN F1 [get_ports seg_en[2]]
set_property IOSTANDARD LVCMOS33 [get_ports seg_en[2]]
set_property PACKAGE_PIN E1 [get_ports seg_en[1]]
set_property IOSTANDARD LVCMOS33 [get_ports seg_en[1]]
set_property PACKAGE_PIN G6 [get_ports seg_en[0]]
set_property IOSTANDARD LVCMOS33 [get_ports seg_en[0]]

set_property PACKAGE_PIN B4 [get_ports seg1[7]]
set_property IOSTANDARD LVCMOS33 [get_ports seg1[7]]
set_property PACKAGE_PIN A4 [get_ports seg1[6]]
set_property IOSTANDARD LVCMOS33 [get_ports seg1[6]]
set_property PACKAGE_PIN A3 [get_ports seg1[5]]
set_property IOSTANDARD LVCMOS33 [get_ports seg1[5]]
set_property PACKAGE_PIN B1 [get_ports seg1[4]]
set_property IOSTANDARD LVCMOS33 [get_ports seg1[4]]
set_property PACKAGE_PIN A1 [get_ports seg1[3]]
set_property IOSTANDARD LVCMOS33 [get_ports seg1[3]]
set_property PACKAGE_PIN B3 [get_ports seg1[2]]
set_property IOSTANDARD LVCMOS33 [get_ports seg1[2]]
set_property PACKAGE_PIN B2 [get_ports seg1[1]]
set_property IOSTANDARD LVCMOS33 [get_ports seg1[1]]
set_property PACKAGE_PIN D5 [get_ports seg1[0]]
set_property IOSTANDARD LVCMOS33 [get_ports seg1[0]]

set_property PACKAGE_PIN D4 [get_ports seg0[7]]
set_property IOSTANDARD LVCMOS33 [get_ports seg0[7]]
set_property PACKAGE_PIN E3 [get_ports seg0[6]]
set_property IOSTANDARD LVCMOS33 [get_ports seg0[6]]
set_property PACKAGE_PIN D3 [get_ports seg0[5]]
set_property IOSTANDARD LVCMOS33 [get_ports seg0[5]]
set_property PACKAGE_PIN F4 [get_ports seg0[4]]
set_property IOSTANDARD LVCMOS33 [get_ports seg0[4]]
set_property PACKAGE_PIN F3 [get_ports seg0[3]]
set_property IOSTANDARD LVCMOS33 [get_ports seg0[3]]
set_property PACKAGE_PIN E2 [get_ports seg0[2]]
set_property IOSTANDARD LVCMOS33 [get_ports seg0[2]]
set_property PACKAGE_PIN D2 [get_ports seg0[1]]
set_property IOSTANDARD LVCMOS33 [get_ports seg0[1]]
set_property PACKAGE_PIN H2 [get_ports seg0[0]]
set_property IOSTANDARD LVCMOS33 [get_ports seg0[0]]

set_property PACKAGE_PIN R17 [get_ports reverse_signal_button]
set_property IOSTANDARD LVCMOS33 [get_ports reverse_signal_button]

set_property PACKAGE_PIN M4 [get_ports enable_manual_signal]
set_property IOSTANDARD LVCMOS33 [get_ports enable_manual_signal]

set_property PACKAGE_PIN N4 [get_ports enable_semiauto_signal]
set_property IOSTANDARD LVCMOS33 [get_ports enable_semiauto_signal]

set_property PACKAGE_PIN R1 [get_ports enable_auto_signal]
set_property IOSTANDARD LVCMOS33 [get_ports enable_auto_signal]

set_property IOSTANDARD LVCMOS33 [get_ports red[0]]
set_property IOSTANDARD LVCMOS33 [get_ports red[1]]
set_property IOSTANDARD LVCMOS33 [get_ports red[2]]
set_property IOSTANDARD LVCMOS33 [get_ports red[3]]
set_property IOSTANDARD LVCMOS33 [get_ports green[0]]
set_property IOSTANDARD LVCMOS33 [get_ports green[1]]
set_property IOSTANDARD LVCMOS33 [get_ports green[2]]
set_property IOSTANDARD LVCMOS33 [get_ports green[3]]
set_property IOSTANDARD LVCMOS33 [get_ports blue[0]]
set_property IOSTANDARD LVCMOS33 [get_ports blue[1]]
set_property IOSTANDARD LVCMOS33 [get_ports blue[2]]
set_property IOSTANDARD LVCMOS33 [get_ports blue[3]]
set_property IOSTANDARD LVCMOS33 [get_ports hs]
set_property IOSTANDARD LVCMOS33 [get_ports vs]

set_property PACKAGE_PIN F5 [get_ports red[0]]
set_property PACKAGE_PIN C6 [get_ports red[1]]
set_property PACKAGE_PIN C5 [get_ports red[2]]
set_property PACKAGE_PIN B7 [get_ports red[3]]
set_property PACKAGE_PIN B6 [get_ports green[0]]
set_property PACKAGE_PIN A6 [get_ports green[1]]
set_property PACKAGE_PIN A5 [get_ports green[2]]
set_property PACKAGE_PIN D8 [get_ports green[3]]
set_property PACKAGE_PIN C7 [get_ports blue[0]]
set_property PACKAGE_PIN E6 [get_ports blue[1]]
set_property PACKAGE_PIN E5 [get_ports blue[2]]
set_property PACKAGE_PIN E7 [get_ports blue[3]]
set_property PACKAGE_PIN D7 [get_ports hs]
set_property PACKAGE_PIN C4 [get_ports vs]
