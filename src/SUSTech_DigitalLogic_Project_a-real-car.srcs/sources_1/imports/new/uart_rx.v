...
begin
    ...
    if (clk_16x_cnt > 'd152 && clk_16x_cnt <= 'd168)  //检验停止位条件
      next_state = stop;
    if (clk_16x_cnt > 'd168) next_state = idle;
  end
    ...
    if (rst) begin
      ...
      case (present_state)
        idle: begin  //检测开始位
          ...
          else if (start_flag == 1) clk_16x_cnt <= clk_16x_cnt + 'd1;
        end
        one: begin  //接收8位数据
        ...
        stop: begin  //停止位
          ...
          if (clk_16x_cnt > 168) begin
            clk_16x_cnt <= 0;
            start_flag  <= 0;
          end else clk_16x_cnt <= clk_16x_cnt + 'd1;
        end
      endcase
    end
  end
